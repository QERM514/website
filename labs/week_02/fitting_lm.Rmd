...
This week we will see how to fit linear models in **R**, and estimate parameter uncertainty and confidence intervals. The data set we will use comes from a paper by Johnson & Raven (1973, *Science* 179:893-895) and consists of the diversity of plants species across 30 of the Galapagos Islands, along with measurements of 5 possible explanatory variables:
...
The `gala` dataset are included in the **faraway** package. Let's load the package and inspect the dataset 
...
<center>species diversity = $f$(area).</center>
...
Note that these are the same estimates that we obtained above.
...
We saw in lecture that we can the *coefficient of determination* $(R^2)$ to measure how well a model fits the data. The formula is
...
Increasing the dimension of $\mathbf{X}$ by adding more predictors (columns) necessitates an increased space within which to minimize this objective function. We can demonstrate this with a simple example. Suppose we added another predictor to our simple model that was nothing more than a vector of Gaussian white noise, which is unrelated to the response.
...
Notice that our $R^2$ value increased.

To account for this, we can use an *adjusted* $R^2$, which accounts for these additional degrees of freedom.
...
Notice that our estimates for the intercept $\beta_0$ and the effect of island area $\beta_1$ have changed when we accounted for additional predictors in the model.
...
Here we can see that we got the same $F$-statistic and $p$-value ias above.


### Testing one predictor

We might ask whether any one predictor could be dropped from our model. For example, can $\text{nearest}$ be dropped from this model? One option is to fit these two models and compare them via our $F$-test with $H_0: \beta_3 = 0$

$$
\begin{aligned}
\text{species}_i &= \beta_0 + \beta_1 \text{area}_i + \beta_2 \text{elevation}_i + \beta_3 \text{nearest}_i + e_i \\
~ \\ 
\text{species}_i &= \beta_0 + \beta_1 \text{area}_i + \beta_2 \text{elevation}_i + e_i
\end{aligned}
$$
...
## reduced model without `elevation` and `nearest`
reduced_mod <- lm(Species ~ Area, gala)
## use `anova('reduced', 'full')` to get the F-test results
anova(reduced_mod, full_mod)
```

The $F$-statistic is big and the $p$-value is small so we reject $H_0$
...
The $F$-statistic is large and the $p$-value is small so we reject $H_0$
...
We can also modify our $t$-test from before and use it for our comparison

$$
t_i = \frac{(\beta_i - c)}{\text{SE} \left( \beta_i \right)}
$$

We get the estimate of $\beta_{\text{elevation}}$ and $\text{SE}(\beta_{\text{elevation}})$ from our model summary
...
This leaves us with $\beta_{\text{elevation}} \approx 0.17$ and $\text{SE}(\beta_{\text{elevation}}) \approx 0.055$. 
...
We can also use confidence intervals (CI's) to express uncertainty in $\beta_i$, which take the form
...
We can construct a 95% CI on $\beta_{\text{area}}$ using the 2.5% and 97.5% percentiles of the $t$-distribution with $df = 30 - 4 = 26$.
...
The above $F$- and $t$-based CI's depend on the assumption of normality. Recall that the bootstrap<sup>$\dagger$</sup> method provides a way to construct CI's without this assumption
...
We saw in lecture that we can also estimate the uncertainty for a *specific* prediction rather than the average response. Let's do so for the same hypothetical 2000 km<sup>2</sup> island. Recall that a PI is given by 
...
Notice how much wider this interval is than the CI we estimated above. We could instead use `predict()` as we did above for the CI by switching the `interval` argument.
