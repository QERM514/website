...
data_links_html <- sapply(lapply(data_links, function(x) {
  if(length(x)>1) {
    tmp1 <- paste0("[", as.character(fontawesome::fa("file-table", height = 20, fill = "#eb6864")), " (", 1:length(x), ")](")
    } else {
      tmp1 <- paste0("[", as.character(fontawesome::fa("file-table", height = 20, fill = "#eb6864")), "](")
      }
  tmp2 <- ")"
  tmp <- paste0(tmp1, x, tmp2)
  tmp[x==""] <- ""
