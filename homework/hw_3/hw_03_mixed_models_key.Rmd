...
You can find the R Markdown file used to create this answer key [here](hw_03_mixed_models_key.Rmd).
...
There are many answers to this question but I'm looking for something along these lines:

1) The intrinsic growth rate for each stock $i$ $(\alpha_i)$ is correlated among stocks and can be modeled as a random effect.

2) The intrinsic growth rate scaled by carrying capacity for each stock $i$ $(\beta_i)$ is correlated among stocks and can be modeled as a random effect.

3) Both the intrinsic growth rate for each stock $i$ $(\alpha_i)$ and the intrinsic growth rate scaled by carrying capacity for each stock $i$ $(\beta_i)$ are correlated among stocks and can be modeled as random effects.
...
Here is a plot using base **R**:

```{r b_plot_data, echo = TRUE, fig.height=4.5, fig.width=4, fig.align='center'}
## load the data
...
There appears to be a generally positive relationship between the number of spawning salmon and the number of offspring recruiting some years later. The data also show signs of log-normal heteroscasticity where the variance increases with the mean.
...
This model assumes *all* populations have the same intrinsic growth rate $(\alpha)$ **and** the same intrisic growth rate scaled by carrying capacity $(\beta)$. This is an example of "complete pooling".

Here is the model fit to the data:

```{r c_fit_base_model, echo = TRUE}
## calculate new response variable: log(R/S)
psc$logRS <- log(psc$recruits / psc$spawners)

## fit base model with global parameters
...
Here is a modified model that includes a random effect of population $(\delta_i)$ on the intrinsic growth rate.
...
Here's an example of how to fit the RE model:

```{r d_fit_RE_popn_alpha, echo = TRUE, message = FALSE}
## load {lme4} library
library(lme4)

## fit RE model for alpha
...
The estimate of $\alpha$ is `r round(summary(mod_re_popn_alpha)$coef[1,1], 2)` and $\beta$ is `r round(summary(mod_re_popn_alpha)$coef[2,1], 3)`. The estimates of $\delta_i$ for each of the `r n_pops` populations are
...
The trick here is to recognize that you only want a random effect for the slope $\eta$, and not the intercept, which means you need to specify the random effect as

`(-1 + spawners | pop_n)`

Here is the RE model fit to the data:

```{r e_fit_RE_popn_beta, echo = TRUE, message = FALSE}
## fit RE model for beta
...
The estimate of $\alpha$ is `r round(summary(mod_re_popn_beta)$coef[1,1], 2)` and $\beta$ is `r round(summary(mod_re_popn_beta)$coef[2,1], 3)`. The estimates of the $\eta_i$ for each of the `r n_pops` populations are
...
The $R^2$ value for this model is ~`r round(R2, 3)`, which is much better than that for (c) and about twice that for (d), but still quite low.
...
Here you want *uncorrelated* random effects for both the intercept and slope, which means you need to specify the random effects as 

`(1 + spawners || pop_n)`

Here is this RE model fit to the data:

```{r f_mod_re_popn_both, echo = TRUE, message = FALSE}
## fit RE for beta
...
The estimate of $\alpha$ is `r round(summary(mod_re_popn_both)$coef[1,1], 2)` and $\beta$ is `r round(summary(mod_re_popn_both)$coef[2,1], 3)`. The estimates of both the $\delta_i$ and $\eta_i$ for each of the `r n_pops` populations are
...
The $R^2$ value for this model is only ~`r round(R2, 3)`, which is slightly worse than that for (e).
...
Here is the test for the population-level offset to the intercept:

```{r g_LRT_site, warning=FALSE}
## load RLRsim package
library(RLRsim)
...
```{r g_LRT_year, warning=FALSE}
## test RE for slope
exactRLRT(mod_re_popn_beta, mod_re_popn_both, mod_re_popn_alpha)
```

Here, too, we can reject $H_0 : \sigma_{\eta}^2 = 0$ and conclude that there is support for inclusion of the population-level offset to the density-dependent effect.
...
Here you want *uncorrelated* random effects for both the intercept and slope, plus a random effect for year, which means you need to specify the random effects as 

`(1 + spawners || pop_n) + (1 | year)`

Here is the model fit to the data:

```{r h_mod_re_popn_3, echo = TRUE, message = FALSE}
## fit RE model for alpha, beta & year
mod_re_popn_3 <- lmer(logRS ~ 1 + spawners + (1 + spawners || pop_n)  + (1 | year),
                      data = psc)
...
The estimate of $\alpha$ is `r round(summary(mod_re_popn_both)$coef[1,1], 2)` and $\beta$ is `r round(summary(mod_re_popn_both)$coef[2,1], 3)`. 

The estimates of the $\delta_i$ (`pop_n$(Intercept)`), $\eta_i$ (`pop_n$spawners`), and $\gamma_t$ (`year$(Intercept)`) are
...
Here are the variance estimates:
...
```{r i_re_diagnostics, fig.width=7, fig.height=4, fig.align='center'}
## set plot area
par(mai = c(0.9, 0.9, 0.6, 0.1),
    omi = c(0, 0, 0, 0),
...
```{r i_re_diagnostics_2, fig.width=4, fig.height=4.5, fig.align='center'}
## resids vs fitted
