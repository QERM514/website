...
\Downarrow \\
0.5 = \frac{1}{1 + \exp(\text{-}[14 - 0.17 L_{0.5}])} \\
\Downarrow \\
...
We have seen that we can express the results from a logistic regression in terms of the *odds*. The odds $o$ are an unbounded alternative to probability $p$, such that if the odds are $k$-to-1 against something, which we can write as $o = 1/k$, then

$$
p = \frac{1}{1 + k} ~ \Rightarrow ~ k = \frac{1 - p}{p}
$$

<br>

For example, if $k$ = 4, then $p = \frac{1}{1 + 4} = 0.2$. If $p$ = 0.8, then $k = \frac{1 - 0.8}{0.8} = \frac{1}{4}$.

In a logistic regression model, the logit function gives us the log odds, such that 
...
points(df[70,"length"], df[70, "age"] + 1, pch = 1, cex = 2)
text(df[70,"length"], df[70, "age"] + 1, "70", pos = 3, offset = 0.8)
points(df[72,"length"], df[72, "age"] + 1, pch = 1, cex = 2)
text(df[72,"length"], df[72, "age"] + 1, "72", pos = 1, offset = 0.8)
