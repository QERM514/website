...
Load the **{faraway}** package and inspect the data.

:::

```{r load_gala_dataset, class.source = 'fold-show'}
## load package
library(faraway)
...
```{r plot_gala, echo = FALSE, fig.height = 4, fig.width = 4.5, fig.align = "center", fig.alt = "Bivariate plot of the number of species on an island versus the area of the island in kilometers."}
...
## Estimating $\boldsymbol{\widehat{\beta}}$
...
Next create the matrix of predictors $\mathbf{X}$. There is only one predictor in the model (`Area`), so our matrix should consist of a column of 1's and a column with the area of each island.
...
Estimate the regression parameters in $\boldsymbol{\widehat{\beta}}$.
...
## Estimating $\mathbf{\widehat{y}}$

Recall that the estimated (fitted) values are given by

$$
\begin{align}
\mathbf{\widehat{y}} &= \mathbf{X} \boldsymbol{\widehat{\beta}} \\
...
Calculate the hat matrix $\mathbf{H}$ and solve for $\mathbf{\widehat{y}}$
...
```{r plot_gala_fit, fig.height = 4, fig.width = 4.5, fig.align = "center", fig.alt = "Bivariate plot of the number of species on an island versus the area of the island in kilometers, including the estimated regression line through the data."}
par(mai = c(0.9, 0.9, 0.3, 0.1), omi = rep(0, 4))
## plot the data
plot(gala$Area, gala$Species, pch = 16, col = "dark green",
...
We can obtain the estimates of the fitted values $\mathbf{\widehat{y}}$ via one of two functions in **R**: `fitted()` and `predict()`. We'll see later that `predict()` has additional functionality that we can use.
...
Create a new predictor variable that's just white noise and fit a new regression model that includes both `Area` and the white noise component. Check the adjusted $R^2$ value.
...
### Test *all* predictors in a model
...
$$
C: \mathbf{y} = \mathbf{X} \boldsymbol{\beta} + \mathbf{e} \\
~ \\
S: \mathbf{y} = \boldsymbol{\mu} + \mathbf{e} 
$$
...
$$
F = \frac{ \left( SSE_S - SSE_C \right)  / (k - 1) } { SSE_C  / (n - k)}
$$

with

$$
SSE_{C} = \left( \mathbf{y} - \mathbf{X} \boldsymbol{\beta} \right)^{\top} \left( \mathbf{y} - \mathbf{X} \boldsymbol{\beta} \right) \\
SSE_{S} = \left( \mathbf{y} - \bar{y} \right)^{\top} \left( \mathbf{y} - \bar{y} \right)
$$
...
Calculate the SSE for both models, the subsequent $F$ statistic, and the p-value.
...
We can instead use `anova(simple_model, complex_model)` to conduct this $F$-test.
...
$$
\begin{aligned}
C: \text{species}_i &= \beta_0 + \beta_1 \text{area}_i + \beta_2 \text{elevation}_i + \beta_3 \text{nearest}_i + e_i \\
~ \\ 
S: \text{species}_i &= \beta_0 + \beta_1 \text{area}_i + \beta_2 \text{elevation}_i + e_i
\end{aligned}
$$
...
C: \text{species}_i &= \beta_0 + \beta_1 \text{area}_i + \beta_2 \text{elevation}_i + \beta_3 \text{nearest}_i + e_i \\
~ \\ 
S: \text{species}_i &= \beta_0 + \beta_2 \text{elevation}_i + e_i
\end{aligned}
$$
...
Again we can conduct this test by fitting both models and calculating $F$ via `anova()`.
...
The $F$-statistic is rather small and the $p$-value is big so we cannot reject $H_0$, suggesting we would be justified in dropping area and nearest from the model.
...
We can also modify our $t$-test from before and use it for our comparison by including the hypothesized $\widehat{\beta}_{H_0}$ as an offset.
...
We can get $\widehat{\beta}_{\text{elevation}}$ and $\text{SE}(\widehat{\beta}_{\text{elevation}})$ from our model summary.
...
This leaves us with $\widehat{\beta}_{\text{elevation}} \approx 0.171$ and $\text{SE}(\widehat{\beta}_{\text{elevation}}) \approx 0.0545$. 
...
We can also use confidence intervals (CI's) to express uncertainty in $\widehat{\beta_i}$, which take the form

$$
100(1 - \alpha)\% ~ \text{CI}: \widehat{\beta_i} \pm t_{n-p}^{(\alpha / 2)} \operatorname{SE} ( \widehat{\beta} )
$$
...
We can construct a 95% CI on $\widehat{\beta}$ using the 2.5% and 97.5% percentiles of the $t$-distribution with $df = 30 - 4 = 26$.
...
Step 2: Calculate $\mathbf{e} = \mathbf{y} - \mathbf{X} \boldsymbol{\widehat{\beta}}$ 
...
Step 3b: Calculate $\mathbf{y}^* = \mathbf{X} \boldsymbol{\widehat{\beta}} + \mathbf{e}^*$  
Step 3c: Estimate $\boldsymbol{\widehat{\beta}}^*$ from $\mathbf{X}$ & $\mathbf{y}^*$  
...
Step 4: Select the $\tfrac{\alpha}{2}$ and $(1 - \tfrac{\alpha}{2})$ percentiles from the saved $\boldsymbol{\widehat{\beta}}^*$
...
$$
\mathbf{\widehat{y}}^* \pm ~ t^{(\alpha / 2)}_{df} \sigma \sqrt{ {\mathbf{X}^*}^{\top} (\mathbf{X}^{\top} \mathbf{X})^{-1} \mathbf{X}^* } 
$$
