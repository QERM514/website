...
output:
  pdf_document:
    includes:
      before_body: title_page_template.tex
    highlight: haddock
    toc: true
...
Below is a table with a caption above it. The table itself consists of a `data.frame`, which is formatted for nicer output with `kable()` and `kable_styling`. You specify the caption with the `caption` argument to `kable()` as seen in the example code block below

You can find lots of different options for formatting your tables here:

https://cran.r-project.org/web/packages/kableExtra/vignettes/awesome_table_in_html.html. 

```{r tbl_example, echo = TRUE}
...
## print table
knitr::kable(tbl_ex,
             caption = "This is a basic table caption.",
             format = "latex",
             booktabs = TRUE,
...
You can add a figure caption below a figure using the `fig.cap` argument at the top of an R code block. For example,

````
`r ''````{r fig_ex, fig.height=3, fig.width=5, fig.align='center', echo = TRUE, fig.cap = "An example of a not very nice figure."}
## an ugly plot
par(mai = c(1, 1, 0, 0))
...
```{r fig_ex, fig.height=3, fig.width=5, fig.align='center', echo = TRUE, fig.cap = "An example of a not very nice figure."}
## an ugly plot
par(mai = c(1, 1, 0, 0))
matplot(matrix(rpois(60, 10), 15, 4), las = 1, type = "l",
        xlab = expression(paste("Something here (",hat(italic(y)),")")),
