...
~ \\
f(y ; \mu, \sigma^{2}) = \left( \frac{1}{2 \pi \sigma^{2}} \right)^{1/2} \exp \left[ \text{-} \frac{(y - \mu)^2}{2 \sigma^2} \right]
$$
...
$$
\int^{+\infty}_{\text{-}\infty} f(y) dy = 1
$$
...
\Downarrow \\
\hat{\theta}= \min_{\theta} \text{-}\frac{1}{n} \sum_{i = 1}^n \log f_{\theta}(y_i)
$$
...
f(y_1, \dots, y_n ; \mu, \sigma^{2}) &= \prod_{i = 1}^n f(y_i ; \mu, \sigma^{2}) \\
 &= \left( \frac{1}{2 \pi \sigma^{2}} \right)^{n/2}  \exp \left[ \text{-}  \frac{\sum_{i = 1}^n (y_i - \mu)^2}{2 \sigma^2}  \right]
\end{align}
$$
...
f(y ; \mu, \sigma^{2}) = \left( \frac{1}{2 \pi \sigma^{2}} \right)^{1/2} \exp \left[ \text{-} \frac{(y - \mu)^2}{2 \sigma^2} \right]\\
\Downarrow \\
\log f(y ; \mu, \sigma^{2}) = \text{-}\frac{n}{2} \log(2 \pi \sigma^{2}) -\frac{1}{2 \sigma^2} \sum_{i = 1}^n (y_i - \mu)^2
$$
...
$$
\frac{\partial}{\partial \mu} \log f(y ; \mu, \sigma^{2}) = 0 - \frac{\text{-}2 n (\bar{y} - \mu)}{2 \sigma^2} = 0 \\
\Downarrow \\
\frac{\text{-}2 n (\bar{y} - \mu)}{2 \sigma^2} = 0 \\
\Downarrow \\
...
$$
\frac{\partial}{\partial \sigma} \log f(y ; \mu, \sigma^{2}) = \text{-}\frac{n}{\sigma} - \frac{1}{\sigma^3} \sum_{i = 1}^n (y_i - \mu) = 0 \\
\Downarrow \\
...
$\hat{\boldsymbol{\beta}} = (\mathbf{X}^{\top} \mathbf{X})^{\text{-}1} \mathbf{X}^{\top} \mathbf{y}$
