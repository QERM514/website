...
We say the columns in $\mathbf{X}$ define the *design* of the analysis
...
How would we write $\mathbf{X}$ for a simple model that included a mean only?
...
Let's start by rewriting our model
...
and then redefine $\mathbf{X}$ and $\boldsymbol{\beta}$
...
What about a model with an intercept and linear increase over time $t$?
...
```{r sim_data, fig.align='center', fig.height=4.5, fig.width=5.5}
set.seed(514)
## sample size
nn <- 30
## groups
...
Let's now re-order all of the observations into their 3 ration groups
...
```{r plot_means_only, fig.align='center', fig.height=4.5, fig.width=5.5}
## plot all data
par(mai = c(1, 1,0.1,0.1),
    omi = c(0.1, 0.1, 0, 0.2),
    cex = 1.2)
...
Let's estimate the $\beta_i$'s by hand
...
$\mathbf{X}$ is *rank-deficient* because $\mathbf{X}_{(\cdot 1)} = \mathbf{X}_{(\cdot 2)} + \mathbf{X}_{(\cdot 3)} + \mathbf{X}_{(\cdot 4)}$
...
\Downarrow \\
\mu = \frac{3 \mu + (\beta_1 + \beta_2 + \beta_3)}{3} \\
\Downarrow \\
...
If we substitute for $\beta_3$

$$
(\beta_1 + \beta_2 + \beta_3) = 0 ~ \Rightarrow ~ \beta_3 = \text{-} \beta_1 + \text{-} \beta_2
$$
...
```{r plot_anova_gm, fig.align='center', fig.height=5, fig.width=6}
## plot all data
par(mai = c(1, 1,0.1,0.1),
    omi = c(0.1, 0.1, 0, 0.2),
    cex = 1.2)
...
```{r plot_anova_ref, fig.align='center', fig.height=5, fig.width=6}
## plot all data
par(mai = c(1, 1,0.1,0.1),
    omi = c(0.1, 0.1, 0, 0.2),
    cex = 1.2)
...
```{r plot_ancova, fig.align='center', fig.height=5, fig.width=6}
## plot all data
par(mai = c(1, 1,0.1,0.1),
    omi = c(0.1, 0.1, 0, 0.2),
    cex = 1.2)
## plot all data
plot(x_cov[1:nn], yy[1:nn], pch = 16, col = "red",
     xlim = c(0, 12), ylim = c(10, 45),
...
XX <- cbind(L1 = rep(c(1,0,0), ea = nn), # col 1: effect of lineage 1
            L2 = rep(c(0,1,0), ea = nn), # col 2: effect of lineage 2
            L3 = rep(c(0,0,1), ea = nn), # col 3: effect of lineage 3
            RA = x_cov)                  # col 4: effect of ration
## fit model
Bvec <- coef(lm(yy ~ XX - 1))
...
```{r plot_ancova_fit, fig.align='center', fig.height=5, fig.width=6}
## plot all data
par(mai = c(1, 1, 0.1, 0.1),
    omi = c(0.1, 0.1, 0, 0.2),
    cex = 1.2)
## blank plot
plot(x_cov[1:nn], yy[1:nn], type = "n",
     xlim = c(0, 12), ylim = c(10, 45),
     las = 1, xlab = "Ration size (g)", ylab = "Growth (mm)")
## add fits
