...
<center>"Which of them is the 'best'?"</center>
...
$$
SSE_{S}  = \mathbf{e}^{\top} \mathbf{e} = \left( \mathbf{y} - \bar{y} \right)^{\top} \left( \mathbf{y} - \bar{y} \right) \\
~ \\
SSE_{C}  = \mathbf{e}^{\top} \mathbf{e} = \left( \mathbf{y} - \mathbf{X} \boldsymbol{\beta} \right)^{\top} \left( \mathbf{y} - \mathbf{X} \boldsymbol{\beta} \right)
$$
...
x <- c(-5,5)
par(mai = c(0.9, 0.9, 0.1, 0.1), omi = rep(0, 4))
curve(dt(x, df=2), from=-5, to=5, lwd = 2, col = "orange",
      ylab = "", xlab = expression(italic(x)), las = 1, ylim = c(0, 0.4) )
curve(dt(x, df=20), from=-5, to=5, lwd = 2, col = "red", add = TRUE)
curve(dt(x, df=200), from=-5, to=5, lwd = 2, col = "blue", add = TRUE)
legend(3, 0.35, legend = c("df = 2",
                           "df = 20",
                           "df = 200"),
       col = c("orange", "red", "blue"),
       lwd = 2, bty = "n", xjust = 0.5, y.intersp = 1.2)
```
...
```{r t_vs_norm, fig.height = 4, fig.width = 6, fig.align = "center"}
x <- c(-5,5)
par(mai = c(0.9, 0.9, 0.1, 0.1), omi = rep(0, 4))
curve(dnorm(x, 0, 1), from=-5, to=5, lwd = 6, col = "darkgray",
      ylab = "", xlab = expression(italic(x)), las = 1, ylim = c(0, 0.4) )
curve(dt(x, df = 200), from=-5, to=5, lwd = 2, col = "blue", add = TRUE)
legend(3, 0.35, legend = c("Normal (0, 1)",
                           "t (df = 200)"),
       col = c("darkgray", "blue"),
       lwd = 2, bty = "n", xjust = 0.5, y.intersp = 1.2)
```
...
## Bootstrap pseudocode {.smaller}

```{r ex_bootstrap, eval = FALSE, echo = TRUE}
