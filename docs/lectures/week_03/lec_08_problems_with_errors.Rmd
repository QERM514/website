...
> - Understand how weighted least squares can be used when the errors have non-constant variance

> - Understand how robust methods can be used when the errors are non-normal or when we have influential observations


## Concerns about model assumptions
...
\end{bmatrix},
\begin{bmatrix}
\sigma^2_1 & \gamma_{1,2} & \cdots & \gamma_{1,n} \\
\gamma_{2,1} & \sigma^2_2 & \cdots & \gamma_{2n} \\
\vdots & \vdots & \ddots & \vdots \\
\gamma_{n,1} & \gamma_{n2} & \cdots & \sigma^2_n 
\end{bmatrix} 
\right)
$$
...
* the diagonal contains the variances $\color{blue}{\sigma^2_i}$

* the off-diagonals are the covariances $\gamma_{i,j}$

$$
\boldsymbol{\Sigma} =
\begin{bmatrix}
\color{blue}{\sigma^2_1} & \gamma_{1,2} & \cdots & \gamma_{1,n} \\
\gamma_{2,1} & \color{blue}{\sigma^2_2} & \cdots & \gamma_{2,n} \\
\vdots & \vdots & \ddots & \vdots \\
\gamma_{n,1} & \gamma_{n,2} & \cdots & \color{blue}{\sigma^2_n} 
\end{bmatrix}
$$
...
\boldsymbol{\Sigma} =
\begin{bmatrix}
\sigma^2_1 & \color{purple}{\gamma_{1,2}} & \cdots & \color{red}{\gamma_{1,n}} \\
\color{purple}{\gamma_{2,1}} & \sigma^2_2 & \cdots & \color{teal}{\gamma_{2,n}} \\
\vdots & \vdots & \ddots & \vdots \\
\color{red}{\gamma_{n,1}} & \color{teal}{\gamma_{n,2}} & \cdots & \sigma^2_n 
\end{bmatrix}
$$

Also note that $\boldsymbol{\Sigma}$ is _symmetric_ such that $\gamma_{i,j} = \gamma_{j,i}$
...
## Weighted least squares | When errors are proportional to the predictor

The weights $w_i$ should reflect differences in the variance of the errors $e_i$

In many ecological applications, the variance is proportional to some predictor $x$
...
## Weighted least squares | When observations are averages

Often the observations $y_i$ are averages of several pieces of raw data

In that case

$$
\text{Var}(e_i) = \frac{\sigma^2}{n_i}
$$

This suggests $w_i = n_i$


## Weighted least squares | When observations are sums
...
We saw in the last lecture that non-normal errors & unusual observations can affect model fits, such as

* heteroscedastic errors where $\text{Var}(e_i) \propto n_i$

* outliers that do not come from the data generating process

In these cases we can use so-called *robust regression*
...
and $c = \hat{\sigma} \propto \text{Median}(\left| e \right|)$
...
Note the following about M-estimation:

* it does not address points with large leverage
...
$$
SSE = \sum^n_{i = 1} e^2_{i} ~ \rightarrow ~ SSE_q = \sum^q_{i = 1} e^2_{s(i)}
$$

where $s(i)$ indicates the residuals are sorted in ascending order

<br>

The default is $q = \lfloor n/2 \rfloor + \lfloor (k + 1) / 2 \rfloor$ where $\lfloor \cdot \rfloor$ is the [floor function](https://en.wikipedia.org/wiki/Floor_and_ceiling_functions)
