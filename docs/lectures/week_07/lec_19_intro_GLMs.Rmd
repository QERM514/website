...
* the number of rabbits seen outside my office window each day

> - the number of fish caught in a 15-minute bottom trawl
...
Has _only one_ parameter $\lambda$ to describe both the mean *and* variance
...
The simplest ratio has as denominator of 1 and a numerator of either 0 or 1
...
The Bernoulli distribution describes the probability of a _single event_ $y_i$ occurring
...
It describes the number of "successes" $(k)$ in a sequence of $n$ independent Bernoulli "trials"
...
|:------------:|:-------------:|:-------------:|
| Normal | _Identity_: $1(\mu) = \mathbf{X} \boldsymbol{\beta}$ | $\mu = \mathbf{X} \boldsymbol{\beta}$ |
| Poisson  | _Log_: $\log (\mu) = \mathbf{X} \boldsymbol{\beta}$ | $\mu = \exp (\mathbf{X} \boldsymbol{\beta})$ |
| Binomial  | _Logit_: $\log \left( \frac{\mu}{1 - \mu} \right) = \mathbf{X} \boldsymbol{\beta}$ | $\mu = \frac{\exp (\mathbf{X} \boldsymbol{\beta})}{1 + \exp (\mathbf{X} \boldsymbol{\beta})}$ |


## Common link functions

```{r link_funcs, fig.height=4.5, fig.width=10, fig.align='center'}
## set plot area
par(mfrow = c(1, 3),
...
With GLMs, we can shift our focus to counts as a function of area
...
Survival $s_i$ of $N_i$ salmon eggs to $y_i$ fry as a function of water temperature $T_i$
