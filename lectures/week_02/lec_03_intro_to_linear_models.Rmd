...
Yes, if we allow for a transformation
...
If we reduce the interval of interest enough, we can approximate a nonlinear function with a linear model
...
## Locally linear models | A stochastic example with $y = 2 + \tfrac{1}{2} x + x^2 + \epsilon_i$
...
Discrete predictor $\rightarrow$ **AN**alysis **O**f **VA**riance (ANOVA)

Continuous predictor $\rightarrow$ Regression

Both $\rightarrow$ **AN**alysis of **COVA**riance (ANCOVA)
...
* She wants to know how growth of hatchery salmon is affected by ration size
...
```{r fish_data, fig.align='center', fig.height=4, fig.width=5}
## sample size
nn <- 30
## groups
pp <- 3
...
axis(1, at = seq(3), cex.axis = 1.2,
     labels = c("Low (2)", "Med (6)", "High (10)"))
```
