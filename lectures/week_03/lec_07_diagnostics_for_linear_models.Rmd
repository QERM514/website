...
To check this assumption, we can plot our estimates $\hat{e}_i = e_i = y- \hat{y}$ against our fitted values $\hat{y}_i$ and look for any patterns


## Checking error assumptions | Constant variance

```{r nonconstant_variance, echo = FALSE, fig.width = 9, fig.height = 4, fig.align = "center"}
set.seed(514)
## sample size
nn <- 30
## phony y_hat
...
For a finer resolution, we can also plot $\sqrt{| \hat{e}_i |}$ against our fitted values $\hat{y}_i$ and look for any patterns

The distribution of $| \hat{e}_i |$ is a skewed half-normal on the positive interval; the square-root transformation makes them less skewed 


## Checking error assumptions | Constant variance

```{r nonconstant_variance_2, echo = FALSE, fig.width = 9, fig.height = 4, fig.align = "center"}
...
```{r qq_plots, echo = FALSE, fig.width = 9, fig.height = 4, fig.align = "center"}
## set plot area
par(mfrow = c(1,3),
    mai = c(0.9,0.5,0.5,0.2),
    omi = c(0, 0, 0, 0),
...
Large $h_i$ lead to small variances of $e_i$ & hence $\hat{y}_i$ tends to $y_i$
