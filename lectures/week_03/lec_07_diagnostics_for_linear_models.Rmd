...
We can formally test the assumption of homogeneous variance via *Levene's Test*

$$
W=\frac{(n-k)}{(k-1)} \cdot \frac{\sum_{i=1}^{k} n_{i}\left(Z_{i} - \bar{Z} \right)^{2}}{\sum_{i=1}^{k} \sum_{j=1}^{n_{i}} \left( Z_{i j} - \bar{Z_{i}} \right)^{2}}
$$
...
which are distributed as a $t$ distribution with $n - l - 1$ df
...
$$
t_i = \frac{y_{i} - \hat{y}_{(i)}}{ \hat{\sigma}_{(i)} \sqrt{ 1 - h_i } } = r_i \sqrt{ \frac{n - k - 1}{n - k - r_i^2} }
$$
