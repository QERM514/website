...
```{r nonconstant_variance, echo = FALSE, fig.width = 9.5, fig.height = 4, fig.align = "center"}
set.seed(514)
## sample size
nn <- 30
## phony y_hat
...
```{r nonconstant_variance_2, echo = FALSE, fig.width = 9.5, fig.height = 4, fig.align = "center"}
...
plot(y_hat, e1s, pch = 16, las = 1, xpd = NA,
     cex.lab = 1.5, xlab = "Fitted values", ylab = expression(sqrt(abs(italic(e)))),
     main = "No problem")
...
```{r levene, echo = TRUE}
## split residuals (ee) into 2 groups: less than and greater than the median
g1 <- ee[ee < median(ee)]
g2 <- ee[ee > median(ee)]
## Levene's Test
var.test(g1, g2)
...
## Checking error assumptions | Expected quantiles for $e \sim \text{N}(0,1)$
...
curve(dnorm, -4, 4, las = 1, bty = "n", lwd = 2,
      ylab = "Density", xlab = expression(italic(e)))
...
```{r qq_plots, echo = FALSE, fig.width = 9.5, fig.height = 4, fig.align = "center"}
## set plot area
par(mfrow = c(1,3),
    mai = c(0.9,0.5,0.5,0.2),
    omi = c(0, 0, 0, 0),
...
```{r outliers, echo = FALSE, fig.width = 9.5, fig.height = 4, fig.align = "center"}
...
$\text{var}(e_i) = \sigma^2(1 - h_{i,i})$

where $h_{i,i}$ is the leverage of observation (datum) $i$
...
\mathbf{H} = 
\begin{bmatrix}
\color{blue}{ h_{1,1}} & h_{1,2} & \dots & h_{1,n} \\
h_{2,1} & \color{blue}{h_{2,2}} & \dots & h_{2,n} \\
\vdots & \vdots & \color{blue}{\ddots} & \vdots \\
h_{n,1} & h_{n,2} & \dots & \color{blue}{h_{n,n}}
\end{bmatrix}
$$

with $\color{blue}{h_{i,i}} \leq 1$

## Leverages and residual variance

So given that 

$\text{Var}(e_i) = \sigma^2 (1 - h_{i,i})$ and ${h_{i,i}} \leq 1$

<br>

What happens as $h_{i,i} \rightarrow 1$?
...
```{r outliers_2, echo = FALSE, fig.width = 9.5, fig.height = 4, fig.align = "center"}
## set plot area
par(mfrow = c(1, 3),
    mai = c(0.9, 0.5, 0.5, 0.2),
    omi = c(0, 0.4, 0, 0),
...
$$
\text{trace}(\mathbf{H}) = \sum_{i = 1}^n h_{i,i}
$$

Thus, on average we should expect that $\bar{h}_{i,i} = \frac{k}{n}$

<br>

Any $h_{i,i} > \frac{2 k}{n}$ deserve closer inspection


## Identifying leverage points

We can easily compute the $h_{i,i}$ in **R** via the function `hatvalues()`
...
## Identifying leverage points | via a half-normal plot<sup>*</sup>
...
## plots
plot(model.matrix(m2)[,2], hat_values, pch = 16, las = 1,
     ylab = "Leverage", xlab = expression(italic(x)))
mtext(expression(italic(h)^{"*"}), 4, line = 0.3, cex = 1.1, at = threshold, las = 1)
abline(h = threshold, lty = "dashed")
halfnorm(hat_values)
```

<sup>*</sup>see `.Rmd` file for R code to do this
...
par(mai = c(1.1, 1.1, 0.1, 0.1), omi = c(0, 0, 0, 0), cex = 1.2)
## standardized r
...
knitr::include_graphics("peterson_outliers.png")
```

[Peterson et al (2014)](https://www.jstor.org/stable/24862214)
